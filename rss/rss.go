...
func joinBase(baseURL, name string) string {
...
func GenerateRSS(folder string, baseURL string) (*RSS, error) {
	...
	for _, f := range files {
		...
		items = append(items, Item{
			...
			GUID:        guid,
			Enclosure: Enclosure{
				URL:    joinBase(baseURL, encodedName),
				Type:   "audio/mpeg",
	...
	return &RSS{
		...
		Channel: Channel{
			...
			Explicit:    "false",
			Image:       iTunesImage{Href: joinBase(baseURL, "image.png")},
			SelfLink: AtomLink{
				Href: joinBase(baseURL, "rss.xml"),
				Rel:  "self",
